...
class HomeServiceCubit extends Cubit<HomeServiceState> {
  HomeServiceCubit() : super(const HomeServiceState());

  void onAddressChange(String address) {
    address = address.trim();

    final RegExp validAddressRegex = RegExp(r"^(?=.*[a-zA-Z0-9])[a-zA-Z0-9\s,.'-]+$");

    if (address.isEmpty) {
      emit(state.copyWith(addressError: "Address cannot be empty"));
    } else if (!validAddressRegex.hasMatch(address)) {
      emit(state.copyWith(addressError: "Invalid address. Must include letters or numbers."));
    } else {
  ...
  void onAreaChange(String area) {
    ...
    final RegExp validAreaRegex = RegExp(r"^(?=.*[a-zA-Z0-9])[a-zA-Z0-9\s,.'-]+$");

    if (area.isEmpty) {
      emit(state.copyWith(areaError: "Area cannot be empty"));
    } else if (!validAreaRegex.hasMatch(area)) {
      emit(state.copyWith(areaError: "Invalid area. Must include letters or numbers."));
    } else {
  ...
  void onLandmarkChange(String landmark) {
    final RegExp validLandMarkRegex = RegExp(r"^(?=.*[a-zA-Z0-9])[a-zA-Z0-9\s,.'-]+$");
    if (landmark.isEmpty) {
      emit(state.copyWith(landmarkError: "landmark cannot be empty"));
    } else if (!validLandMarkRegex.hasMatch(landmark)) {
      emit(state.copyWith(landmarkError: "Invalid landmark. Must include letters or numbers."));
    } else {
  ...
  void onAllFieldsValid(BuildContext context) {
      if(state.addressError != null && state.areaError != null && state.pinCodeError != null && state.landmarkError != null){
        if(state.addressError != null && state.addressError!.isNotEmpty){
        ...
        } else if(state.landmarkError != null && state.landmarkError!.isNotEmpty){
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
          ...
          Navigator.pushNamed(context, RoutesHelper.homeServiceScreen);
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
        ...
        emit(state.copyWith(addressError: "Address cannot be empty", areaError: "Area cannot be empty", pinCodeError: "Pin code cannot be empty", landmarkError: "Landmark cannot be empty"));
      }
  }
